...
- Default release checks:
  ...
  - `pnpm release:check`
  - `OPENCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
- Use `docs/reference/testing-ci-policy.md` as the source of truth for where
  end-to-end and live suites belong: `PR CI`, `release CI`, `scheduled CI`, or
  `manual only`.
- Keep "required in CI" separate from "blocking the publish workflow." A suite
  that is too slow or flaky for a blocking publish path may still be required in
  release-time or scheduled CI.
- Do not read an "optional local command" as permission to drop that suite from
  CI. Local spot checks and CI placement are different decisions.
