...
const TEST_EXECUTABLE_COVERAGE_ID = "agent-runtime-and-provider-execution.dm";
const TEST_BROWSER_CATEGORY_ID = "browser-control-ui-and-webchat.browser-ui";
const TEST_BROWSER_COVERAGE_ID = "browser-control-ui-and-webchat.gateway-hosted-ui-control";
const TEST_WEBCHAT_COVERAGE_ID = "agent-runtime-and-provider-execution.webchat";
...
describe("qa coverage report", () => {
  it("groups scenario coverage metadata by theme and surface", () => {
    ...
    expect(personalPack?.coverageIds).toContain(
      "security-auth-pairing-and-secrets.redaction-personal-redaction",
    );
    ...
    expect(observabilityPack?.coverageIds).toContain(
      "telemetry-diagnostics-and-observability.prometheus-text-exposition",
    );
    expect(
      expectDefined(inventory.byTheme.memory, "memory QA theme").map((coverage) => coverage.id),
    ).toContain("session-memory-and-context-engine.memory-search-and-store-tools-recall");
    expect(
  ...
  it("renders a compact markdown inventory", () => {
    ...
    expect(report).toContain("session-memory-and-context-engine.embedding-search-recall");
    ...
    expect(report).toContain(
      "- browser-automation-and-exec-sandbox-tools.tool-invocation-and-execution (browser-automation-and-exec-sandbox-tools / Tool Invocation and Execution; partial): profiles: all, release; coverage IDs:",
    );
    expect(report).toContain(
      "primary:playwright:ui/src/e2e/chat-flow.e2e.test.ts (browser-control-ui-and-webchat.gateway-hosted-ui-control)",
    );
  ...
  it("rejects one coverage ID assigned to different exact features", () => {
    ...
    expect(() =>
      ...
      `coverage ID ${TEST_EXECUTABLE_COVERAGE_ID} already belongs to agent-runtime-and-provider-execution.agent-turn-execution feature shared; coverage IDs must identify exactly one taxonomy feature`,
    );
  });
  ...
  it("requires one two-part ID owned by each exact taxonomy feature", () => {
    ...
    feature.coverageIds = ["agent-runtime-and-provider-execution.delivery.group"];
    expect(() =>
      buildQaScorecardTaxonomyReport({ taxonomy, repoRoot: process.cwd(), scenarios: [] }),
    ...
    expect(() =>
      ...
      `coverage ID ${TEST_BROWSER_COVERAGE_ID} must belong to surface agent-runtime-and-provider-execution`,
    );
  });
