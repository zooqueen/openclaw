...
describe("qa scenario catalog", () => {
  ...
  const telemetry = "telemetry-diagnostics-and-observability";
  ...
  it("loads the YAML pack as the canonical source of truth", () => {
    ...
    expect(readQaScenarioById("memory-recall").coverage?.primary).toContain(
      `${memory}.memory-search-and-store-tools-recall`,
    );
  });
  ...
  it("loads native test execution scenarios from YAML", () => {
    ...
    expect(otelSmoke.coverage?.secondary).not.toContain(`${telemetry}.otlp-http-traces-qa-lab`);
  });
  ...
  it("loads helper-backed HTTP API scenarios as supporting taxonomy coverage", () => {
    ...
    expect(readQaScenarioById("openwebui-openai-compatible").coverage?.secondary).toEqual(
      expect.arrayContaining([
        "gateway-runtime.openai-compatible-apis",
        `${agentRuntime}.hosted-provider-turns`,
      ]),
    );
  });
  ...
  it("loads the QA bus tool trace visibility harness scenario", () => {
    ...
    expect(scenario.coverage?.primary).toContain(
      `${telemetry}.model-and-runtime-telemetry-tool-trace-visibility`,
    );
    expect(scenario.coverage?.secondary ?? []).toStrictEqual([
      `${telemetry}.model-and-runtime-telemetry-qa-bus`,
      `${telemetry}.model-and-runtime-telemetry-trace`,
    ]);
  ...
  it("routes the Codex doctor migration row through the product-backed Vitest", () => {
    ...
    expect(scenario.coverage?.secondary).toEqual([
      `${telemetry}.openclaw-doctor-codex-plugin-auth`,
    ]);
  });
