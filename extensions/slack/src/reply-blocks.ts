import type { ReplyPayload } from "openclaw/plugin-sdk/reply-runtime";
import { parseSlackBlocksInput, SLACK_MAX_BLOCKS } from "./blocks-input.js";
import {
  buildSlackInteractiveBlocks,
  buildSlackPresentationBlocks,
  type SlackBlock,
} from "./blocks-render.js";

function resolveRenderedSlackPresentationBlocks(payload: ReplyPayload): SlackBlock[] {
  const slackData = payload.channelData?.slack;
  if (slackData && typeof slackData === "object" && !Array.isArray(slackData)) {
    const presentationBlocks = (slackData as { presentationBlocks?: unknown }).presentationBlocks;
    if (presentationBlocks != null) {
      if (Array.isArray(presentationBlocks) && presentationBlocks.length === 0) {
        return [];
      }
      return (parseSlackBlocksInput(presentationBlocks) as SlackBlock[]) ?? [];
    }
  }
  return buildSlackPresentationBlocks(payload.presentation);
}

export function resolveSlackReplyBlocks(payload: ReplyPayload): SlackBlock[] | undefined {
  const slackData = payload.channelData?.slack;
  const interactiveBlocks = buildSlackInteractiveBlocks(payload.interactive);
  const presentationBlocks = resolveRenderedSlackPresentationBlocks(payload);
  let channelBlocks: SlackBlock[] = [];
  if (slackData && typeof slackData === "object" && !Array.isArray(slackData)) {
    channelBlocks =
      (parseSlackBlocksInput((slackData as { blocks?: unknown }).blocks) as SlackBlock[]) ?? [];
  }
  const blocks = [...channelBlocks, ...presentationBlocks, ...interactiveBlocks];
  if (blocks.length > SLACK_MAX_BLOCKS) {
    throw new Error(
      `Slack blocks cannot exceed ${SLACK_MAX_BLOCKS} items after interactive render`,
    );
  }
  return blocks.length > 0 ? blocks : undefined;
}
