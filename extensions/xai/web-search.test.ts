...
function unauthorizedResponse(detail: string): Response {
  return new Response(detail, { status: 401, statusText: "Unauthorized" });
}
...
describe("xai web search config resolution", () => {
  ...
  it("refreshes xAI OAuth auth and retries web search after a 401", async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockResolvedValueOnce(unauthorizedResponse("expired"))
      .mockResolvedValueOnce(
        jsonResponse({
          output: [
            {
  ...
  it("falls back to xAI API-key auth when OAuth refresh cannot recover", async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockResolvedValueOnce(unauthorizedResponse("revoked"))
      .mockResolvedValueOnce(
        jsonResponse({
          output: [
            {
  ...
  it("falls back to env auth after a stale xAI API-key auth profile returns unauthorized", async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockResolvedValueOnce(unauthorizedResponse("stale api key"))
      .mockResolvedValueOnce(
        jsonResponse({
          output: [
            {
