...
describe("openai-compatible generic embedding provider", () => {
  it("registers through the plugin entry as a generic embedding provider", () => {
    ...
    expect(plugin.id).toBe("openai-compatible-embeddings");
    expect(registeredProviders).toEqual([openAICompatibleEmbeddingProviderAdapter]);
  });
  ...
  it("reports missing required config with actionable keys", () => {
    expect(() =>
      createOpenAICompatibleEmbeddingProvider(
        createOptions({ remote: { baseUrl: "   " }, model: "text-embedding-bge-m3" }),
      ),
    ).toThrow("remote.baseUrl");
    expect(() =>
      createOpenAICompatibleEmbeddingProvider(
