...
describe("voice-call plugin", () => {
  ...
  it("CLI rejects invalid numeric options", async () => {
    ...
    await expect(
      program.parseAsync(["voicecall", "latency", "--last", "later"], { from: "user" }),
    ).rejects.toThrow(/Invalid numeric value for --last/);
    await expect(
      program.parseAsync(["voicecall", "latency", "--last", "Infinity"], { from: "user" }),
    ).rejects.toThrow(/Invalid numeric value for --last/);
    await expect(
      program.parseAsync(["voicecall", "latency", "--last", "1.5"], { from: "user" }),
    ).rejects.toThrow(/Invalid numeric value for --last/);
  });
