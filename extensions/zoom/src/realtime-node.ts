...
import { resolveZoomRealtimeAudioFormat, resolveZoomRealtimeProvider } from "./realtime.js";
...
export async function startNodeRealtimeAudioBridge(params: {
  ...
  const stop = async () => {
    ...
    try {
      ...
      params.logger.debug?.(
        `[zoom] node realtime bridge close ignored: ${formatErrorMessage(error)}`,
      );
    }
    try {
      ...
      params.logger.debug?.(`[zoom] node audio bridge stop ignored: ${formatErrorMessage(error)}`);
    }
  };
  ...
  bridge = createRealtimeVoiceBridgeSession({
    ...
    audioSink: {
      ...
      sendAudio: (audio) => {
        ...
        void params.runtime.nodes
          ...
          .catch((error) => {
            params.logger.warn(`[zoom] node audio output failed: ${formatErrorMessage(error)}`);
            void stop();
          });
      },
      clearAudio: () => {
        ...
        void params.runtime.nodes
          ...
          .catch((error) => {
            params.logger.warn(`[zoom] node audio clear failed: ${formatErrorMessage(error)}`);
            void stop();
          });
      },
    },
    ...
    onError: (error) => {
      params.logger.warn(`[zoom] node realtime voice bridge failed: ${formatErrorMessage(error)}`);
      void stop();
    },
    onClose: (reason) => {
      realtimeReady = false;
    ...
    for (;;) {
      ...
      try {
        ...
        if (!stopped) {
          ...
          params.logger.warn(
            `[zoom] node audio input failed (${consecutiveInputErrors}/5): ${message}`,
          );
