...
import {
  consultOpenClawAgentForZoom,
  ZOOM_AGENT_CONSULT_TOOL_NAME,
  resolveZoomRealtimeTools,
  submitZoomConsultWorkingResponse,
...
export function resolveZoomRealtimeAudioFormat(config: ZoomConfig) {
  return config.chrome.audioFormat === "g711-ulaw-8khz"
    ? REALTIME_VOICE_AUDIO_FORMAT_G711_ULAW_8KHZ
    : REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ;
}

export function resolveZoomRealtimeProvider(params: {
  config: ZoomConfig;
...
export async function startCommandRealtimeAudioBridge(params: {
  ...
  const stop = async () => {
    ...
    try {
      ...
      params.logger.debug?.(
        `[zoom] realtime voice bridge close ignored: ${formatErrorMessage(error)}`,
      );
    }
  ...
  const fail = (label: string) => (error: Error) => {
    params.logger.warn(`[zoom] ${label} failed: ${formatErrorMessage(error)}`);
    void stop();
  };
  const attachOutputProcessHandlers = (proc: BridgeProcess) => {
    ...
    proc.on("exit", (code, signal) => {
      ...
      if (!stopped) {
        params.logger.warn(`[zoom] audio output command exited (${code ?? signal ?? "done"})`);
        void stop();
      }
    });
    proc.stderr?.on("data", (chunk) => {
      params.logger.debug?.(`[zoom] audio output: ${String(chunk).trim()}`);
    });
  };
  const clearOutputPlayback = () => {
    ...
    params.logger.debug?.(
      `[zoom] cleared realtime audio output buffer by restarting playback command`,
    );
  ...
  inputProcess.on("exit", (code, signal) => {
    if (!stopped) {
      params.logger.warn(`[zoom] audio input command exited (${code ?? signal ?? "done"})`);
      void stop();
    }
  });
  attachOutputProcessHandlers(outputProcess);
  inputProcess.stderr?.on("data", (chunk) => {
    params.logger.debug?.(`[zoom] audio input: ${String(chunk).trim()}`);
  });

  const resolved = resolveZoomRealtimeProvider({
  ...
  bridge = createRealtimeVoiceBridgeSession({
    ...
    audioFormat: resolveZoomRealtimeAudioFormat(params.config),
    instructions: params.config.realtime.instructions,
    initialGreetingInstructions: params.config.realtime.introMessage,
    triggerGreetingOnReady: false,
    markStrategy: "ack-immediately",
    tools: resolveZoomRealtimeTools(params.config.realtime.toolPolicy),
    audioSink: {
      isOpen: () => !stopped,
      sendAudio: (audio) => {
    ...
    onTranscript: (role, text, isFinal) => {
      if (isFinal) {
        ...
        params.logger.debug?.(`[zoom] ${role}: ${text}`);
      }
    },
    onToolCall: (event, session) => {
      if (event.name !== ZOOM_AGENT_CONSULT_TOOL_NAME) {
        session.submitToolResult(event.callId || event.itemId, {
          error: `Tool "${event.name}" not available`,
        });
        return;
      }
      submitZoomConsultWorkingResponse(session, event.callId || event.itemId);
      void consultOpenClawAgentForZoom({
        config: params.config,
        fullConfig: params.fullConfig,
