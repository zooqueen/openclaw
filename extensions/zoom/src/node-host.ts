...
import { DEFAULT_ZOOM_AUDIO_INPUT_COMMAND, DEFAULT_ZOOM_AUDIO_OUTPUT_COMMAND } from "./config.js";
import { ZOOM_SYSTEM_PROFILER_COMMAND, outputMentionsBlackHole2ch } from "./transports/chrome.js";
...
function startChrome(params: Record<string, unknown>) {
  ...
  const needsAudioBridge = Boolean(bridgeCommand) || params.mode === "realtime";
  if (needsAudioBridge) {
  ...
  if (params.launch !== false) {
    ...
    if (result.code !== 0) {
      ...
      throw new Error(
        `failed to launch Chrome for Zoom: ${result.stderr || result.stdout || result.code}`,
      );
    }
  }
...
function normalizeZoomKey(value?: string): string | undefined {
  if (!value) {
    return undefined;
  }
  try {
    ...
    if (url.protocol !== "https:" || !(host === "zoom.us" || host.endsWith(".zoom.us"))) {
      return value;
    }
    const meeting = /^\/(?:j|wc\/join|s)\/(\d{6,14})(?:\/)?$/i.exec(url.pathname);
    const webClient = /^\/wc\/(\d{6,14})\/join(?:\/)?$/i.exec(url.pathname);
    const personal = /^\/my\/([A-Za-z0-9._-]+)(?:\/)?$/i.exec(url.pathname);
    const meetingKey = meeting?.[1]
      ? `id:${meeting[1]}`
      : webClient?.[1]
        ? `id:${webClient[1]}`
        : personal?.[1]
          ? `my:${personal[1].toLowerCase()}`
          : undefined;
    if (!meetingKey) {
      return value;
    }
    return `${host}:${meetingKey}:${url.searchParams.get("pwd") ?? ""}`;
  } catch {
    return value;
  }
}
...
function listSessions(params: Record<string, unknown>) {
  const urlKey = normalizeZoomKey(readString(params.url));
  const mode = readString(params.mode);
  const bridges = [...sessions.values()]
    .filter((session) => !session.closed)
    .filter((session) => !urlKey || normalizeZoomKey(session.url) === urlKey)
...
function stopSessionsByUrl(params: Record<string, unknown>) {
  ...
  for (const [bridgeId, session] of sessions) {
    ...
    if (normalizeZoomKey(session.url) !== urlKey) {
      continue;
    }
...
export async function handleZoomNodeHostCommand(paramsJSON?: string | null): Promise<string> {
  ...
  switch (action) {
    ...
    case "stop":
      ...
      break;
    default:
      throw new Error("unsupported zoom.chrome action");
  }
