...
//
// IN events are fed straight into the reply-dispatcher wiring (dispatcher
// options, delivery callbacks, and replyOptions callbacks); OUT
...
function setupFeishuTrace(recorder: WireRecorder, scenario: DeliveryTraceScenarioName) {
  ...
  const { delivery, dispatcherOptions: options } = created;
