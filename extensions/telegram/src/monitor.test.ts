...
describe("monitorTelegramProvider (grammY)", () => {
  ...
  it("rebuilds the resolved transport after a stalled polling cycle", async () => {
    ...
    try {
      ...
      resolveTelegramTransportSpy
        .mockReturnValueOnce(initialTransport)
      ...
      expect(createTelegramBotCalls[0]?.telegramTransport).toBe(initialTransport);
  ...
  it("rebuilds the resolved transport after unhandled polling network rejection", async () => {
    const initialTransport = {
      fetch: globalThis.fetch,
      sourceFetch: globalThis.fetch,
    };
