...
// request- or connection-scoped, so remembering the current one a gateway method
// receives is sound. The Control UI calls `workspaces.get` on load, so the slot is
// populated long before an agent edits anything.

export type WorkspaceBroadcast = (event: string, payload: unknown) => void;

let handle: WorkspaceBroadcast | undefined;

/** Called by every workspace gateway method to retain the current server broadcast. */
