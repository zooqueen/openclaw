...
- Plugin APIs, provider routing, auth/session state, persisted preferences, config loading, migrations, setup, startup checks, and fallback behavior are compatibility/upgrade-sensitive. Treat config breaks, removed fallbacks, fail-closed changes, or new operator action as merge risk even with green CI. New or changed config/default additions are `reviewMetrics` and merge-risk material when they can affect existing users, upgrades, provider/plugin behavior, persisted preferences, setup, startup, workflow behavior, fallback behavior, or operator action. When practical, the metric should name the count and direction (added, changed, removed) of config/default surface changes and why those changes affect review risk.
