...
describe("install runtime enforcement", () => {
  it.each(["22.22.2", "22.22.3", "23.11.0", "24.14.1", "24.15.0", "25.8.1", "25.9.0", "26.0.0"])(
    ...
    (version) => {
      expect(nodeVersionSatisfiesPackageEngine(version, readPackageNodeEngine())).toBe(
  ...
  it("fails closed when the package engine contract cannot be read", () => {
    const reportError = vi.fn();
    expect(
      enforceSupportedNodeRuntime(
        {
    ...
    expect(reportError).toHaveBeenCalledWith(
      expect.stringContaining("could not read this OpenClaw release's Node requirement"),
    );
  });
});
