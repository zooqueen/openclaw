...
describe("package acceptance workflow", () => {
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(workflow).toContain(
      '[[ ( "$CHILD_WORKFLOW_REF" == release-ci/* || "$CHILD_WORKFLOW_REF" == "dev/throwaway-2026.0.33-v6.8" || "$CHILD_WORKFLOW_REF" =~ ^extended-stable/[0-9]{4}\\.([1-9]|1[0-2])\\.33$ ) && -n "${TARGET_SHA// }" && "$head_sha" != "$TARGET_SHA" ]]',
    );
    expect(workflow).toContain(
