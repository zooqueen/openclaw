...
describe("scripts/test-parallel fatal output guard", () => {
  it("auto-isolates unit files that mock shared runtime or config modules", () => {
    const catalog = loadTestCatalog();
    const classification = catalog.classifyTestFile("src/cli/program/config-guard.test.ts");

    expect(classification.surface).toBe("unit");
    expect(classification.isolated).toBe(true);
    expect(classification.reasons).toContain("auto-shared-module-mock");
  });
...
describe("scripts/test-parallel lane planning", () => {
  ...
  it("uses earlier targeted channel batching on high-memory local hosts", () => {
    ...
    expect(Math.max(...channelBatchFilterCounts)).toBeLessThan(40);
