...
function selectProviderRuntimeContractFixtures(providerIds: readonly string[]) {
  const selectedFixtures = PROVIDER_RUNTIME_CONTRACT_FIXTURES.filter((fixture) =>
    fixture.providerIds.some((providerId) => providerIds.includes(providerId)),
  );
  if (selectedFixtures.length === 0) {
    throw new Error(
      `provider runtime contract fixture missing for ${providerIds.join(", ") || "(none)"}`,
    );
  }
...
function installRuntimeHooks(providerIds: readonly string[]) {
  beforeAll(async () => {
    providerRuntimeContractProviders.clear();
    const registeredFixtures = await Promise.all(
      selectProviderRuntimeContractFixtures(providerIds).map(async (fixture) => {
...
export function describeGithubCopilotProviderRuntimeContract() {
  describe(
    ...
    () => {
      installRuntimeHooks(["github-copilot"]);
...
export function describeGoogleProviderRuntimeContract() {
  describe("google provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    installRuntimeHooks(["google", "google-gemini-cli"]);
...
export function describeOpenAIProviderRuntimeContract() {
  describe("openai provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    installRuntimeHooks(["openai", "openai-codex"]);
...
export function describeOpenRouterProviderRuntimeContract() {
  describe("openrouter provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    installRuntimeHooks(["openrouter"]);
