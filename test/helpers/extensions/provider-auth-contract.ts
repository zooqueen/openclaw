...
async function loadOpenAICodexProvider() {
  const token = Date.now();
  const [{ default: openAIPlugin }, codexProvider] = await Promise.all([
    import(`${providerAuthContractModules.openAIIndexModuleUrl}?t=${token}`) as Promise<{
      default: Parameters<typeof registerProviders>[0];
    }>,
    import(`${providerAuthContractModules.openAICodexProviderModuleUrl}?t=${token}`) as Promise<{
      __setLoadLoginOpenAICodexOAuthForTest: (
        loader: () => typeof loginOpenAICodexOAuthMock,
      ) => void;
    }>,
  ]);
  codexProvider.__setLoadLoginOpenAICodexOAuthForTest(() => loginOpenAICodexOAuthMock);
...
export function describeOpenAICodexProviderAuthContract() {
  ...
  describe("openai-codex provider auth contract", () => {
    ...
    async function getProvider() {
      return await loadOpenAICodexProvider();
    }
        ...
        __testing: {
          ...
          setDepsForTest: (
            overrides: Partial<{
              ...
              githubCopilotLoginCommand: typeof githubCopilotLoginCommandMock;
            }>,
          ) => void;
        };
        default: Parameters<typeof registerProviders>[0];
