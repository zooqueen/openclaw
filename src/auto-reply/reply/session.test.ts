...
describe("initSessionState browser tab cleanup", () => {
  ...
  it("skips browser tab cleanup when root browser support is disabled", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const sessionRowsTarget = await createSessionRowsTarget("openclaw-tab-cleanup-browser-disabled-");
  ...
  it("skips browser tab cleanup when the browser plugin entry is disabled", async () => {
    ...
    const sessionRowsTarget = await createSessionRowsTarget(
      "openclaw-tab-cleanup-browser-plugin-disabled-",
    );
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("accounts goal usage when fresh token snapshots are persisted", async () => {
    const sessionRowsTarget = await createSessionRowsTarget("openclaw-usage-goal-");
    const sessionKey = "main";
    await seedSessionStore({
      sessionKey,
      entry: {
  ...
  it("clears stale CLI binding when usage update reports an unflushed replacement", async () => {
    const sessionRowsTarget = await createSessionRowsTarget("openclaw-usage-cli-clear-");
    const sessionKey = "main";
    await seedSessionStore({
      sessionKey,
      entry: {
    ...
    const stored = readSessionRowsForFixtureTarget(sessionRowsTarget);
    // cliSessionIds is seeded as a keyed Record; narrow off the string[] union arm to index by CLI id.
    const cliSessionIds = stored[sessionKey].cliSessionIds as Record<string, string> | undefined;
    expect(cliSessionIds?.["claude-cli"]).toBeUndefined();
    expect(cliSessionIds?.["codex-cli"]).toBe("codex-session");
