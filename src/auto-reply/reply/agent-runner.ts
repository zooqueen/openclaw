...
function resolveReplyRunDeliveryContext(params: {
  ...
  const threadId =
    ...
    normalizeOptionalString(
      resolveLoadedSessionThreadInfo(params.sessionCtx.SessionKey ?? params.sessionKey).threadId,
    );
...
export async function runReplyAgent(params: {
  ...
  const persistRestartRecoveryDeliveryContext = async (): Promise<void> => {
    ...
    const persisted = await writeSessionEntryRow({
      sessionKey,
      fallbackEntry: activeSessionEntry,
      sessionStore: activeSessionStore,
  ...
  const clearRestartRecoveryDeliveryContext = async (): Promise<void> => {
    if (!trackedRestartRecoveryDeliveryContext || !sessionKey) {
      return;
    }
    const patch: Partial<SessionEntry> = {
