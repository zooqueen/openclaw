...
describe("readPostCompactionContext", () => {
  ...
  async function expectLegacySectionFallback(
    ...
    const options =
      postCompactionSections === undefined
        ? undefined
        : {
            cfg: {
              agents: {
    ...
    const result = await readPostCompactionContext(tmpDir, options);
  ...
  async function readPostCompactionContextWithOptions(options?: {
    cfg?: OpenClawConfig;
    agentId?: string;
    nowMs?: number;
  }) {
    return readPostCompactionContext(tmpDir, options);
  }
  ...
  it("returns null when AGENTS.md has no relevant sections", async () => {
    ...
    const result = await readPostCompactionContextWithOptions();
    expect(result).toBeNull();
  });
...
`;
  ...
  it("honors per-agent post-compaction context limit overrides", async () => {
    ...
    const result = await readPostCompactionContextWithOptions({ cfg, agentId: "writer" });
    expect(result).toContain("[truncated]");
...
`;
    ...
    const result = await readPostCompactionContextWithOptions({ cfg, nowMs });
...
`;
    fs.writeFileSync(path.join(tmpDir, "AGENTS.md"), content);
    const nowMs = Date.UTC(2026, 2, 3, 14, 0, 0);
    const result = await readPostCompactionContextWithOptions({ nowMs });
  ...
  describe("agents.defaults.compaction.postCompactionSections", () => {
    it("uses default sections when postCompactionSections is not configured", async () => {
      const content = `## Session Startup\n\nDo startup.\n\n## Red Lines\n\nDo not break.\n\n## Other\n\nIgnore.\n`;
      fs.writeFileSync(path.join(tmpDir, "AGENTS.md"), content);
      const result = await readPostCompactionContext(tmpDir);
      expect(result).toContain("Session Startup");
    ...
    it("falls back to legacy sections when postCompactionSections is not configured", async () => {
      await expectLegacySectionFallback(undefined);
    });
