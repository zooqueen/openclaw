...
describe("update swap coverage", () => {
  it("enables swap only for a verified install-cli managed npm prefix", async () => {
  ...
  it("warns for ordinary npm globals", async () => {
    ...
    expect(coverage.protection).toBe("detect-warn");
    expect(formatUpdateSwapCoverageWarning(coverage)).toContain("reinstall the previous version");
  });
});
