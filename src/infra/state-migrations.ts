...
function resolveStaleLegacySessionFile(params: {
  ...
  const legacyBackupHasTranscript = safeReadDir(path.dirname(params.legacyDir)).some(
    (dirEntry) =>
      ...
      dirEntry.name.startsWith(`${path.basename(params.legacyDir)}.legacy-`) &&
      fileExists(
        path.join(path.dirname(params.legacyDir), dirEntry.name, path.basename(legacySessionFile)),
      ),
  );
  ...
  const hasCollisionRename = safeReadDir(params.targetDir).some(
    (dirEntry) =>
      dirEntry.isFile() &&
      dirEntry.name.startsWith(`${parsed.name}.legacy-`) &&
      dirEntry.name.endsWith(parsed.ext),
  );
