...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("imports legacy cron run-log files into SQLite", async () => {
    ...
    const { readCronRunLogEntriesSync } = await import("../../../cron/run-log.js");
    expect(
      readCronRunLogEntriesSync({ storePath: resolveCronStoreKey(), jobId: "stateful-job" }),
  ...
  it("does not auto-repair in non-interactive mode without explicit repair approval", async () => {
    ...
    expect((await loadCronStore(resolveCronStoreKey())).jobs).toEqual([]);
