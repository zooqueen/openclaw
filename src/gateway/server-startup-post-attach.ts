...
function resolveSessionDirsFromDatabaseTargets(
  targets: readonly { databasePath: string }[],
): string[] {
  const dirs = new Set<string>();
  for (const target of targets) {
    dirs.add(path.join(path.dirname(path.dirname(target.databasePath)), "sessions"));
  }
  return [...dirs];
}
...
export async function startGatewaySidecars(params: {
  ...
  schedulePostReadySidecarTask({
    ...
    run: async (isStopped) => {
      try {
        const [{ resolveAllAgentSessionDatabaseTargetsSync }, { cleanStaleLockFiles }] =
          await Promise.all([
            import("../config/sessions/targets.js"),
            import("../agents/session-write-lock.js"),
          ]);
        const sessionDirs = resolveSessionDirsFromDatabaseTargets(
          resolveAllAgentSessionDatabaseTargetsSync(params.cfg),
        );
