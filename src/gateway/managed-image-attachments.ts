...
// `agentId` has no dedicated column; it only survives inside `record_json`.
// Recover it here so agent-scoped cleanup of "global" records still works.
function parseRecordJsonAgentId(recordJson: string): string | undefined {
  try {
    const parsed = JSON.parse(recordJson) as { agentId?: unknown };
...
function rowToManagedImageRecord(row: ManagedImageRecordRow): ManagedImageRecord {
  const agentId = parseRecordJsonAgentId(row.record_json);
  return {
...
async function getSessionManagedOutgoingAttachmentIndex(
  ...
  // Caller-provided agentId disambiguates "global" sessions; otherwise derive
  // the owning agent from the session key so we read the right SQLite store.
  const resolvedAgentId =
    (sessionKey === "global" && agentId?.trim()) ||
    resolveAgentIdFromSessionKey(sessionKey) ||
    DEFAULT_AGENT_ID;
  const loaded = stateDir
    ? undefined
    : loadSessionEntry(sessionKey, sessionKey === "global" && agentId ? { agentId } : undefined);
  const entry =
    loaded?.entry ??
    (stateDir
      ? getSessionEntry({
  ...
  const messages = await readSessionMessagesAsync(
    {
      ...
      ...(loaded?.databasePath
        ...
        : stateDir
          ? { path: managedImageAgentDbPath(resolvedAgentId, stateDir) }
          : {}),
    },
    {
      mode: "full",
