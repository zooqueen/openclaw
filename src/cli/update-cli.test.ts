...
describe("update-cli", () => {
  ...
  it("does not require downgrade confirmation when latest target lookup is unresolved", async () => {
    const tempDir = createCaseDir("openclaw-update");
    setTty(false);
    mockPackageInstallStatus(tempDir);
    readPackageVersion.mockResolvedValue("2026.4.22");
  ...
  it("requires downgrade confirmation when a non-latest dist-tag lookup is unresolved", async () => {
    const tempDir = createCaseDir("openclaw-update");
    setTty(false);
    mockPackageInstallStatus(tempDir);
    readPackageVersion.mockResolvedValue("2026.4.22");
