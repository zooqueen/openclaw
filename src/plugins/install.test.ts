...
describe("installPluginFromArchive", () => {
  ...
  it("surfaces plugin scanner findings from before_skill_install", async () => {
    ...
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_skill_install", handler }]),
    );
    ...
    expect(handler.mock.calls[0]?.[0]).toMatchObject({
      skillName: "hook-findings-plugin",
      builtinFindings: [],
    });
    expect(
      warnings.some((w) =>
  ...
  it("blocks plugin install when before_skill_install rejects after builtin critical findings", async () => {
    const handler = vi.fn().mockReturnValue({
      block: true,
    ...
    expect(handler).toHaveBeenCalledTimes(1);
    expect(handler.mock.calls[0]?.[0]).toMatchObject({
      skillName: "dangerous-blocked-plugin",
      builtinFindings: [
