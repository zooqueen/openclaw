...
export function loadBundledPluginPublicArtifactModuleSync<T extends object>(params: {
  ...
  const shouldWarnLateRuntimeDeps =
    isReplyCapableChannelsLive() && params.installRuntimeDeps !== false;
  const startedAt = shouldWarnLateRuntimeDeps ? Date.now() : 0;
  ...
  if (shouldWarnLateRuntimeDeps) {
    logReplyRuntimeColdPathViolation({
      kind: "public-artifact-runtime-deps",
      source: `plugins.public-surface-loader:${params.artifactBasename}`,
      durationMs: Date.now() - startedAt,
      detail: `pluginId=${params.dirName}`,
    });
  }
  const cached =
