...
const ENUM_EXPECTATIONS: Record<string, string[]> = {
  "agents.defaults.memory.citations": ['"auto"', '"on"', '"off"'],
  "agents.defaults.memory.backend": ['"builtin"', '"qmd"'],
  "agents.defaults.memory.qmd.searchMode": ['"query"', '"search"', '"vsearch"'],
...
describe("config help copy quality", () => {
  ...
  it("explains memory citations mode semantics", () => {
    const help = FIELD_HELP["agents.defaults.memory.citations"];
  ...
  it("includes concrete examples on path and interval fields", () => {
    expect(FIELD_HELP["agents.defaults.memory.qmd.paths.pattern"].includes("**/*.md")).toBe(true);
