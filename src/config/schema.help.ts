...
export const FIELD_HELP: Record<string, string> = {
  ...
  "agents.list[].experimental.localModelLean":
    'Per-agent override for lean local-model mode. Set true to force the trim for one smaller local-model agent, "auto" to trim only when the resolved model context cap is a binary 64K window or smaller, or false to opt out of an inherited default.',
  ...
  "agents.defaults.experimental.localModelLean":
    'Experimental local-model prompt trim. Set true to drop heavyweight default tools like browser, cron, and message for weaker local-model backends, or "auto" to do so only for resolved model context caps at a binary 64K window or smaller.',
