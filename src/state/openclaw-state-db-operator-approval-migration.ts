...
function repairOperatorApprovalKinds(db: DatabaseSync): boolean {
  ...
  // The owning doctor repair wraps every schema migration in one immediate
  // transaction, so this copy/drop/rename joins that atomic unit. Starting a
  // second transaction here would make the repair fail before copying rows.
