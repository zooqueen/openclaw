...
describe("doctor runtime tool schema checks", () => {
  ...
  it("reports active bundle MCP tool schemas that would be quarantined before a model turn", async () => {
    mocks.createBundleMcpToolRuntime.mockReturnValueOnce({
      tools: [
        ...
        bundleMcpTool("fuzzplugin__fuzzplugin_move_angles", {
          type: "array",
          items: { type: "number" },
        }),
      ],
    ...
    await expect(
      ...
      severity: "error",
      message:
        "Agent main tool fuzzplugin__fuzzplugin_move_angles from plugin bundle-mcp has an unsupported input schema for runtime projection.",
      path: "mcp.servers",
      target: "fuzzplugin__fuzzplugin_move_angles",
      requirement: 'fuzzplugin__fuzzplugin_move_angles.parameters.type must be "object"',
      fixHint:
  ...
  it("loads bundled MCP runtime once per distinct agent workspace", async () => {
    ...
    await expect(
      ...
      severity: "error",
      message:
        "Agent worker tool fuzzplugin__fuzzplugin_move_angles from plugin bundle-mcp has an unsupported input schema for runtime projection.",
      path: "mcp.servers",
      target: "fuzzplugin__fuzzplugin_move_angles",
      requirement: 'fuzzplugin__fuzzplugin_move_angles.parameters.type must be "object"',
      fixHint:
