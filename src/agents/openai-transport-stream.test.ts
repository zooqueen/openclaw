...
describe("openai transport stream", () => {
  ...
  it("merges developer prompt back into system for responses providers that disable developer role", () => {
    ...
    expect(params.input?.[0]?.content).toBe("system\n\ndeveloper");
    expect(params.input).toHaveLength(1);
  });

  it("emits separate system and developer roles for native OpenAI responses models", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.4",
        name: "GPT-5.4",
    ...
    expect(params.input?.[0]).toMatchObject({ role: "system", content: "system" });
    expect(params.input?.[1]).toMatchObject({ role: "developer", content: "developer" });
  });
  ...
  it("emits separate system and developer roles for native OpenAI completions models", () => {
    const params = buildOpenAICompletionsParams(
      {
        id: "gpt-5.4",
        name: "GPT-5.4",
    ...
    expect(params.messages).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ role: "developer", content: "developer" }),
      ]),
    );
  });
