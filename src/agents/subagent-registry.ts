...
async function completeSubagentRunWithRecovery(params: CompleteSubagentRunParams, source: string) {
  try {
    ...
    log.warn(
      ...
      {
        event: "agents.subagent.completion",
        outcome: "warning",
        reason: "failed",
      },
    );
  ...
  try {
    ...
    log.warn(
      ...
      {
        event: "agents.subagent.complete.subagent.run",
        outcome: "warning",
        reason: "failed",
      },
    );
...
async function notifyContextEngineSubagentEnded(params: {
  ...
  try {
    ...
    log.warn(
      ...
      { err },
      {
        event: "agents.subagent.engine.subagent_end",
        outcome: "warning",
        reason: "failed",
      },
    );
...
const subagentLifecycleController = createSubagentRegistryLifecycleController({
  ...
  warn: (message, meta) =>
    log.warn(message, meta, {
      event: "agents.subagent.registry.lifecycle",
      outcome: "warning",
      reason: "warning",
    }),
});
...
function restoreSubagentRunsOnce() {
  ...
  try {
    ...
    log.warn(
      ...
      undefined,
      {
        event: "agents.subagent.registry.restore.subagent.runs.disk",
        outcome: "warning",
        reason: "failed",
      },
    );
...
async function discardSuspendedPendingFinalDelivery(
  ...
  log.warn(
    ...
    {
      event: "agents.subagent.suspended_delivery",
      outcome: "warning",
      reason: "discarded",
    },
  );
  ...
  try {
    ...
    if (suspendedEntries.length > SUSPENDED_DELIVERY_HARD_CAP) {
      ...
      log.warn(
        ...
        {
          event: "agents.subagent.suspended.backlog",
          outcome: "warning",
          reason: "cap_exceeded",
        },
      );
    ...
    for (const [runId, entry] of subagentRuns.entries()) {
      ...
      try {
        ...
        log.warn(
          ...
          {
            event: "agents.subagent.sweep.delete_sessions",
            outcome: "warning",
            reason: "failed",
          },
        );
  ...
  listenerStop = subagentRegistryDeps.onAgentEvent((evt) => {
    void (async () => {
      ...
      log.warn(
        ...
        { err, runId: evt.runId },
        {
          event: "agents.subagent.registry.lifecycle.event.handler",
          outcome: "warning",
          reason: "failed",
        },
      );
