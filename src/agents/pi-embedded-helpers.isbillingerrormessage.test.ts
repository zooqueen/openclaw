...
describe("classifyFailoverReason provider messages", () => {
  ...
  it("classifies internal and compatibility error messages", () => {
    ...
    expect(
      classifyFailoverReason(
        'No API key found for provider "openai". Auth store: /tmp/openclaw-state/openclaw.sqlite#table/auth_profile_stores//tmp/openclaw-agent-abc (agentDir: /tmp/openclaw-agent-abc).',
      ),
    ).toBe("auth");
...
describe("classifyProviderRuntimeFailureKind", () => {
  ...
  it("classifies OAuth refresh timeouts and lock contention distinctly", () => {
    ...
    expect(
      classifyProviderRuntimeFailureKind(
        "OAuth token refresh failed for openai-codex: SQLite busy timeout for /tmp/openclaw-state/openclaw.sqlite#table/auth_profile_stores//tmp/agent. Please try again or re-authenticate.",
      ),
    ).toBe("auth_refresh");
  });
