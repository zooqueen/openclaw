...
describe("buildAgentSystemPrompt", () => {
  ...
  it("uses repo-scoped workspace guidance in coding contexts", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      runtimeInfo: { repoRoot: "/tmp/openclaw" },
    });
    ...
    expect(prompt).not.toContain(
      "do not assume it is a software project or create git history unless the user asked or the task is clearly repo-scoped",
    );
  });

  it("uses product-safe workspace guidance outside coding contexts", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    });

    expect(prompt).toContain(
      "Use the current workspace carefully, but do not assume it is a software project or create git history unless the user asked or the task is clearly repo-scoped.",
    );
    expect(prompt).not.toContain(
      "Treat this directory as the single global workspace for file operations unless explicitly instructed otherwise.",
    );
