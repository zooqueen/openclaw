...
import { __testing as toolImagesTesting } from "./tool-images.js";
import { __testing as gatewayTesting } from "./tools/gateway.js";
...
beforeEach(async () => {
  ...
  gatewayTesting.setDepsForTest({ callGateway });
  toolImagesTesting.setDepsForTest({
    getImageMetadata: vi.fn(async () => ({ width: 1, height: 1 })),
    resizeToJpeg: vi.fn(async () => Buffer.from("jpeg")),
  });
  openClawToolsTesting.setDepsForTest({
    callGateway,
    createCanvasTool: () => stubTool("canvas") as never,
    resolvePluginTools: () => [],
  });
  vi.unstubAllGlobals();
});
  ...
  it("requests approval and retries with allow-once decision", async () => {
    ...
    setupSystemRunGateway({
      onRunInvoke: (invokeParams) => {
        ...
        expect(invokeParams.params.runId).not.toBe(approvalId);
