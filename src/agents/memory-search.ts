...
export function resolveMemorySearchConfig(
  ...
  if (
    ...
    throw new Error(
      "agents.*.memory.search.multimodal requires a provider adapter that supports multimodal embeddings for the configured model.",
    );
  }
  if (multimodalActive && resolved.fallback !== "none") {
    throw new Error(
      'agents.*.memory.search.multimodal does not support memory.search.fallback. Set fallback to "none".',
    );
  }
  return resolved;
}
