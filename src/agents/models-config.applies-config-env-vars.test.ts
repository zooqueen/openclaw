...
describe("models-config", () => {
  it(
    "uses config env.vars entries for implicit provider discovery without mutating process.env",
    { timeout: 20_000 },
    async () => {
  ...
  it(
    "does not overwrite already-set host env vars while ensuring models.json",
    { timeout: 20_000 },
    async () => {
      await withTempHome(async () => {
        await withTempEnv(["OPENROUTER_API_KEY", TEST_ENV_VAR], async () => {
