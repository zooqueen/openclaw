...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const hasWorkspaceBootstrap = hookAdjustedBootstrapFiles.some(
      (file) => file.name === DEFAULT_BOOTSTRAP_FILENAME && !file.missing,
    );
    const isCodingContext = Boolean(params.runtime?.repoRoot);
    const workspaceNotes = hasWorkspaceBootstrap && isCodingContext
