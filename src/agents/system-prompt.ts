...
function buildAssistantOutputDirectivesSection(isMinimal: boolean) {
  ...
  return [
    ...
    "- To request a native reply/quote on supported surfaces, include reply tags in your reply.",
    "- A reply tag must be the very first token in the text it applies to: [[reply_to_current]] your reply.",
    "- [[reply_to_current]] replies to the triggering message. Prefer this for single-message replies.",
    "- [[reply_to:<message_id>]] replies to a specific message by id. Use it only when an id was explicitly provided (e.g. in conversation info JSON or by a tool).",
    "- Multiple tags are allowed: when answering queued messages from different senders, place [[reply_to:<message_id>]] before each section so each reply targets the correct message.",
