...
export async function updateSessionEntryAfterAgentRun(params: {
  ...
  if (entry.sessionId !== sessionId) {
    // Track the rotated session id in the usage family so cumulative usage
    // spans the old and new session ids. Storage is SQLite-only, so no
    // sessionFile is rewritten here.
