...
export async function resolveApiKeyForProvider(params: {
  ...
  if (provider === "openai") {
    ...
    if (hasCodex) {
      throw new Error(
        'No API key found for provider "openai". You are authenticated with OpenAI Codex OAuth. Use openai-codex/gpt-5.3-codex (ChatGPT OAuth) or set OPENAI_API_KEY for openai/gpt-5.1-codex.',
      );
    }
  }
