...
export function createExtensionHostPluginRegistrationActions(params: {
  ...
  const registerHook = (
    ...
    const existingHook = registry.hooks.find(
      (entry) => entry.entry.hook.name === normalized.hookName,
    );
    if (existingHook) {
      pushExtensionHostRegistryDiagnostic({
        registry,
        ...
        pluginId: record.id,
        source: record.source,
        message: `hook already registered: ${normalized.hookName} (${existingHook.pluginId})`,
      });
      return;
    }
    addExtensionLegacyHookRegistration({
  ...
  const registerCli = (
    ...
    const existing = listExtensionHostCliRegistrations(registry).find((entry) =>
      entry.commands.some((command) => result.commands.includes(command)),
    );
    if (existing) {
      const overlap = result.commands.find((command) => existing.commands.includes(command));
      pushExtensionHostRegistryDiagnostic({
        registry,
        level: "error",
        pluginId: record.id,
        source: record.source,
        message: `cli command already registered: ${overlap ?? result.commands[0]} (${existing.pluginId})`,
      });
      return;
    }
    addExtensionCliRegistration({
  ...
  const registerService = (record: PluginRecord, service: OpenClawPluginService) => {
    ...
    const existing = listExtensionHostServiceRegistrations(registry).find(
      (entry) => entry.service.id === result.serviceId,
    );
    if (existing) {
      pushExtensionHostRegistryDiagnostic({
        registry,
        ...
        pluginId: record.id,
        source: record.source,
        message: `service already registered: ${result.serviceId} (${existing.pluginId})`,
      });
      return;
    }
    addExtensionServiceRegistration({
