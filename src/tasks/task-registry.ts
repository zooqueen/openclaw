...
function scheduleTaskFlowSyncRetry(task: TaskRecord, operation: string, attempt = 0): void {
  ...
  if (delayMs == null) {
    log.warn(
      ...
      {
        event: "tasks.registry.parent_flow_sync",
        outcome: "warning",
        reason: "exhausted",
      },
    );
  ...
  const retryTimer = setTimeout(() => {
    ...
    if (!result.ok) {
      log.warn(
        ...
        {
          event: "tasks.registry.parent.flow.sync.task",
          outcome: "warning",
          reason: "failed",
        },
      );
...
function syncFlowFromTaskAfterTaskMutation(task: TaskRecord, operation: string): void {
  ...
  log.warn(
    ...
    {
      event: "tasks.registry.sync.parent.flow.task.mutation",
      outcome: "warning",
      reason: "failed",
    },
  );
...
function restoreTaskRegistryOnce() {
  ...
  try {
    ...
    log.warn(
      ...
      { error },
      {
        event: "tasks.registry.restore.task.registry",
        outcome: "warning",
        reason: "failed",
      },
    );
...
function updateTask(taskId: string, patch: Partial<TaskRecord>): TaskRecord | null {
  ...
  try {
    ...
    log.warn(
      ...
      {
        event: "tasks.finalize.managed.flow.cancellation.task.update",
        outcome: "warning",
        reason: "failed",
      },
    );
  ...
  try {
    ...
    if ((shouldRouteParentReview && !shouldDeliverParentReviewDirect) || !canDeliverDirect) {
      try {
        ...
        log.warn(
          ...
          {
            event: "tasks.registry.queue.background.task.session.delivery",
            outcome: "warning",
            reason: "failed",
          },
        );
    ...
    try {
      ...
      log.warn(
        ...
        {
          event: "tasks.registry.deliver.background.task.update",
          outcome: "warning",
          reason: "failed",
        },
      );
      try {
        ...
        log.warn(
          ...
          {
            event: "tasks.registry.queue.background.task.event",
            outcome: "warning",
            reason: "failed",
          },
        );
...
export async function maybeDeliverTaskStateChangeUpdate(
  ...
  try {
    ...
    log.warn(
      ...
      {
        event: "tasks.registry.deliver.background.task.state.change",
        outcome: "warning",
        reason: "failed",
      },
    );
  ...
  if (store.listTasksForOwnerKey) {
    try {
      ...
      log.warn(
        ...
        {
          event: "tasks.registry.read",
          outcome: "warning",
          reason: "owner_records_failed",
        },
      );
    }
  }
