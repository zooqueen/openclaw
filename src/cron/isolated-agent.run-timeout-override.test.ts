...
describe("runCronIsolatedAgentTurn — explicit per-run timeout signal", () => {
  ...
  // Oversized explicit payload timeouts must be clamped to the timer-safe
  // ceiling before they reach the embedded runner, so the watchdog never
  // arms a timer beyond what the platform can represent.
  it("caps oversized explicit payload timeoutSeconds at the timer-safe ceiling", async () => {
    await withTempCronHome(async (home) => {
      await seedCronSessionRows(home, {
        "agent:main:main": {
          sessionId: "main-session",
      ...
      const call = lastEmbeddedCall();
      expect(call.runTimeoutOverrideMs).toBe(MAX_TIMER_TIMEOUT_MS);
    });
  });
});
