---
title: "Mattermost, LINE, IRC, Nextcloud Talk, Nostr, Twitch, Tlon, Synology Chat - Channel Setup and Operations Maturity Note"
version: 3
last_refreshed: 2026-06-01
last_refreshed_by: codex
---

# Mattermost, LINE, IRC, Nextcloud Talk, Nostr, Twitch, Tlon, Synology Chat - Channel Setup and Operations Maturity Note

## Summary

This note is the active normalized maturity note for `Channel Setup and Operations` on the `Mattermost, LINE, IRC, Nextcloud Talk, Nostr, Twitch, Tlon, Synology Chat` channel surface. It consolidates prior channel-specific evidence notes while preserving those older notes in the inventory directory for historical detail.
...
- Nextcloud Talk bot installation: Covers Nextcloud Talk bot installation, shared secret/API credentials, webhook route, room settings, file-backed secrets, and plugin runtime setup behavior.
- Webhook ingress: Covers Webhook ingress, signature/secret validation, room-vs-DM lookup, DM/group policy, pairing, mention gating, replay protection, and room metadata behavior.
- Outbound markdown/text: Covers Outbound markdown/text, URL media fallback, reactions/message actions, threading, status, doctor, setup, and troubleshooting behavior.
...
- Twitch bot account setup: Covers Twitch bot account setup, OAuth access/refresh tokens, client ID/secret, channel join config, multi-account config, and package/bundled install behavior behavior.
- Twitch IRC monitor/client lifecycle: Covers Twitch IRC monitor/client lifecycle, token refresh, status/probe, access control by user ID/roles, `requireMention`, and outbound chat delivery behavior.
- Message tool send action: Covers Message tool send action, moderation-oriented action surface, safety/ops, and troubleshooting behavior.
