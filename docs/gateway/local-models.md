...
3. **Try lean mode.** If both probes pass but real agent turns fail with malformed tool calls or oversized prompts, enable `agents.defaults.experimental.localModelLean: true`. The default `basic` profile drops the three heaviest default tools (`browser`, `cron`, `message`) so the prompt shape is smaller and less brittle. If that still leaves the model confused, add `localModelLeanProfile: "strict"` to keep only the minimal coding/status tools. See [Experimental Features → Local model lean mode](/concepts/experimental-features#local-model-lean-mode) for the full explanation, when to use it, and how to confirm it is on.
