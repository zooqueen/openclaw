...
3. **Try lean mode.** If both probes pass but real agent turns fail with malformed tool calls or oversized prompts, enable `agents.defaults.experimental.localModelLean: "auto"` or `true`. Auto mode drops the three heaviest default tools (`browser`, `cron`, `message`) only for resolved model context caps at a binary 64K window or smaller; `true` forces the same trim when the model cap is unknown or the backend still needs help. See [Experimental Features → Local model lean mode](/concepts/experimental-features#local-model-lean-mode) for the full explanation, when to use it, and how to confirm it is on.
