...
- **前沿工作流：** 自行通过 `pnpm gateway:watch` 运行 Gateway，然后让 macOS 应用以本地模式接入。
...
目标：开发 TypeScript Gateway，获得热重载，同时保持 macOS 应用界面接入。
...
`gateway:watch` 以监视模式运行 Gateway，TypeScript 文件变更时自动重载。
