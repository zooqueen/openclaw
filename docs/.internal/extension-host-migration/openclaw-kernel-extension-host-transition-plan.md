...
- host-owned runtime registry accessors now route through `src/extension-host/runtime-registry.ts`, and the HTTP-route, gateway-method, CLI, and service slices now keep host-owned storage there with mirrored legacy compatibility views
...
- the HTTP-route, gateway-method, CLI, and service slices now also keep host-owned runtime-registry storage with mirrored legacy compatibility arrays and handler maps
...
- by starting actual low-risk runtime write ownership for channel, provider, gateway-method, HTTP-route, tool, CLI, service, command, context-engine, and hook registrations once normalization helpers were already in place, while keeping duplicate enforcement and lifecycle semantics in legacy owners where they still apply
...
- by introducing host-owned runtime-registry accessors for low-risk runtime consumers first, then moving HTTP-route, gateway-method, CLI, and service storage into that host-owned state while keeping mirrored legacy compatibility arrays and handler maps
