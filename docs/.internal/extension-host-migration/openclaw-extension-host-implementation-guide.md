...
- broader embedding-runtime execution and fallback ownership, remaining media facade and config-helper cleanup, plus remaining TTS config/status and facade cleanup
...
- it now explicitly treats embeddings, media understanding, and TTS as in-progress host-owned subsystem runtimes, with embedding selection and fallback now extracted, media registry, execution, auto-entry selection, active-model fallback, and orchestration now extracted, TTS execution, request setup, preferences, and payload planning now partially extracted, and the remaining embedding execution, media facade and config-helper cleanup, plus TTS config/status cleanup still pending, all with capability routing, typed request envelopes, provider-id normalization, and fallback policy
