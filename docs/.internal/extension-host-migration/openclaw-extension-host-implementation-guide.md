...
- broader embedding-runtime execution and fallback ownership, plus remaining media auto-entry, active-model fallback, and orchestration ownership, plus remaining TTS config/status and facade cleanup
...
- it now explicitly treats embeddings, media understanding, and TTS as in-progress host-owned subsystem runtimes, with embedding selection and fallback now extracted, media execution now partially extracted, TTS execution, request setup, preferences, and payload planning now partially extracted, and the remaining embedding execution, media auto-entry and orchestration ownership, plus TTS config/status cleanup still pending, all with capability routing, typed request envelopes, provider-id normalization, and fallback policy
