...
- host-owned runtime registry accessors now have a host-owned helper boundary in `src/extension-host/runtime-registry.ts`, and the HTTP-route and gateway-method slices now keep host-owned storage there with mirrored legacy compatibility views
