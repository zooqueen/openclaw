...
- host-owned runtime registry accessors now route through `src/extension-host/runtime-registry.ts` ahead of broader catalog-backed registry ownership, and the HTTP-route and gateway-method slices now keep host-owned storage there with mirrored legacy compatibility views
...
- by introducing host-owned runtime-registry accessors for low-risk runtime consumers first, then moving HTTP-route and gateway-method storage into that host-owned state while keeping mirrored legacy compatibility arrays and handler maps before broader catalog publication or arbitration work
