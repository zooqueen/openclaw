...
- broader embedding-runtime execution and fallback ownership, plus remaining media auto-entry, active-model fallback, and orchestration ownership, plus remaining TTS config/status and facade cleanup, should become runtime-internal subsystem registries rather than remaining part of a universal plugin-provider API
...
9. Finish broader embedding-runtime execution and fallback ownership, plus remaining media auto-entry, active-model fallback, and orchestration ownership, plus remaining TTS config/status and facade cleanup, with explicit capability routing and built-in fallback policy.
