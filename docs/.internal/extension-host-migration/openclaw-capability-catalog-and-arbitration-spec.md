...
- broader embedding-runtime execution and fallback ownership, remaining media compatibility-facade cleanup, plus remaining TTS config/status and facade cleanup, should become runtime-internal subsystem registries rather than remaining part of a universal plugin-provider API
...
9. Finish broader embedding-runtime execution and fallback ownership, remaining media compatibility-facade cleanup, plus remaining TTS config/status and facade cleanup, with explicit capability routing and built-in fallback policy.
