...
- host-owned runtime registry accessors now delegate through `src/extension-host/runtime-registry.ts`, and the HTTP-route and gateway-method slices now keep host-owned storage there with mirrored legacy compatibility views
...
- by introducing host-owned runtime-registry accessors for provider, tool, service, CLI, gateway-method, and HTTP-route consumers first, then moving HTTP-route and gateway-method storage into that host-owned state while keeping mirrored legacy compatibility arrays and handler maps
