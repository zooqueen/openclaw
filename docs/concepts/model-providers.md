...
<AccordionGroup>
  ...
  <Accordion title="Proxy-route shaping rules">
    ...
    - For slow local models or remote LAN/tailnet hosts, set `models.providers.<id>.timeoutSeconds`. This extends provider model HTTP request handling, including connect, headers, body streaming, and the total request abort, without increasing the whole agent runtime timeout. If `agents.defaults.timeoutSeconds` or a run-specific timeout is lower, raise that ceiling too; provider timeouts cannot extend the whole run.
    - Model provider HTTP egress filtering is owned by `proxy.enabled` plus the operator-managed external proxy policy. For local, LAN, or tailnet provider endpoints, configure the provider `baseUrl` normally and allowlist that destination in the proxy when managed proxy mode is enabled. OpenClaw no longer exposes a model-provider `request.allowPrivateNetwork` knob.
