...
Before advancing to the next configured model, retry-safe internal model calls can opt into one same-model retry for transient failures such as timeouts, rate limits, overloads, and HTTP 5xx server errors. The retry uses abortable bounded backoff, requires the caller to declare its callback idempotent, and is skipped for cooldown probes so same-provider probe slots still advance promptly to cross-provider fallbacks. Full agent orchestration paths do not opt into this retry because tool execution and outbound delivery can have externally visible side effects.
