...
Before advancing to the next configured model, normal agent runs retry the same model once for retry-safe transient failures such as timeouts, rate limits, overloads, and HTTP 5xx server errors. The retry uses a bounded backoff and is skipped for cooldown probes so same-provider probe slots still advance promptly to cross-provider fallbacks. Internal callers that cannot safely repeat their run callback do not opt into this same-model retry.
