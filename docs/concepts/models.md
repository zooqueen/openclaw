...
- Auto fallback selections are temporary recovery state. They are stored with `modelOverrideSource: "auto"` so active fallback sessions keep using the fallback chain without probing a known-bad primary first. New sessions and subagents use the active fallback until a throttled primary probe succeeds; after that, new work starts from the primary again while existing fallback transcripts stay on their selected fallback until reset.
