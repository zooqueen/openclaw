...
- `context: "fork"` for native sub-agents when the child needs the current
  ...
  Thread-bound native sub-agents also default to `context: "isolated"`; set
  `threadBindings.defaultSpawnContext: "fork"` only when thread spawns should inherit
  the requester transcript.
