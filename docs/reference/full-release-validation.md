...
When evidence cannot be reused, `Verify release source checks` runs the same
deterministic `pnpm check`, test-type, and architecture gates used by npm
preflight against the exact Code SHA. The broad child workflows do not dispatch
until it passes. Focused rerun groups skip this source preflight and rerun only
the selected failed surface.

After the source preflight passes, `Verify Docker runtime image assets` builds
the `runtime-assets` Docker target with
`OPENCLAW_EXTENSIONS=diagnostics-otel,codex` in parallel with the child
workflows. A narrower `rerun_group` skips this Docker preflight.
...
| Source preflight        | **Job:** `Verify release source checks`<br />**Child workflow:** none<br />**Proves:** the exact Code SHA passes the deterministic source checks that npm preflight will enforce later. Runs before broad fan-out only for a fresh `rerun_group=all` validation.<br />**Rerun:** fix the Code SHA, then start a new full validation.                                                                                                                                                                                        |
| Docker assets preflight | **Job:** `Verify Docker runtime image assets`<br />**Child workflow:** none<br />**Proves:** the `runtime-assets` Docker build target still succeeds. Runs after source preflight and in parallel with the child workflows for `rerun_group=all`.<br />**Rerun:** rerun the umbrella with `rerun_group=all`.                                                                                                                                                                                                                |
