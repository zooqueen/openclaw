...
8. If validation fails, fix on the release branch and rerun the smallest failed file, lane, workflow job, package profile, provider, or model allowlist that proves the fix. Rerun the full umbrella only when the changed surface makes prior evidence stale.
   If the next beta candidate matches a checked-in release delta policy, run trusted-main `Release Delta Evidence` against the proven SHA and final SHA. Keep its run ID with the exact-target npm preflight ID. The terminal manifest records one direct source-to-target proof, exact changed paths and artifact hashes, reused successful leaves, fresh path-impacted gates, and explicit skips. It cannot consume another delta manifest. Pre-tag creation records a missing or stale tag without failing; publish workflows independently require the remote tag and release branch to point to the final SHA immediately before promotion.
...
## Delta evidence reuse

Use trusted-main `Release Delta Evidence` only when the exact source and target fit a reviewed JSON policy directly under `.github/release-delta-policies/`. Dispatch it with the full target SHA, the checked-in policy path, and a JSON map from the policy's evidence inputs to native workflow run IDs:

```bash
gh workflow run release-delta-evidence.yml \
...
The policy, not dispatch input, defines the beta version and tag, release branch, changelog base tag, source and baseline SHAs, allowed paths, release-metadata exclusions, evidence roles, and optional exact-target producer jobs. The workflow requires trusted `main` attempt 1, a verified target signature, and the release branch head at the target SHA. It then proves a linear source-to-target range whose terminal commit changes only `CHANGELOG.md`, audits the per-commit path union against the endpoint diff, and confirms the proven source itself differs from its direct baseline only by `CHANGELOG.md`.

For every changed non-metadata path, at least one matching role must be `fresh`, and no matching role may be `reused`. Reused roles are accepted only when none of their impact patterns match the delta and their exact successful native evidence binds the source SHA. The producer schedules only the exact-target jobs declared by fresh producer roles; undeclared producer jobs must skip. Aggregate workflow cancellation does not invalidate a successful leaf, but failed, cancelled, ambiguous, expired, or unbound leaf evidence is rejected.

The terminal manifest records the policy path, blob, and hash; source, terminal-parent, and target SHAs; commit and path audits; runtime-tree hashes; package and ClawHub package hashes; gate, conclusion, run, job, artifact, report, and log identities for every role; impacted paths and reuse rationale; cancelled-aggregate disposition evidence; contribution-ledger proof; and the observed signature, release-ref, and pre-tag state. Package comparison is either canonical source-to-target equivalence or fresh exact-target validation as the policy requires. The source npm, target npm, and Telegram package roles are distinct single-run inputs with fixed workflow, job, artifact, report, and binding contracts. An exact-target npm preflight artifact remains authoritative.

Delta manifests cannot be evidence inputs, so reuse is always one direct hop. Publish consumers load the policy from the authenticated producer run; recompute the delta, trust bundle, ClawHub roster, changelog, and release notes; verify the exact promotable npm bytes against the recorded target hashes; and re-fetch the release branch, commit signature, and tag peel immediately before promotion. Missing, stale, moved, or mismatched release state fails closed.
...
- `full_release_validation_run_id`: successful `Full Release Validation` run id for this tag/SHA, required for real publish unless a terminal delta run is supplied
- `release_delta_evidence_run_id`: optional successful trusted-main terminal delta run for an audited beta delta; exact-target `preflight_run_id` remains required
...
- `full_release_validation_run_id`: successful `Full Release Validation` run id; required when `publish_openclaw_npm=true` unless a terminal delta run is supplied
- `release_delta_evidence_run_id`: optional successful trusted-main terminal delta run for an audited beta delta; publication still targets the checked-out tag SHA and exact-target npm preflight artifact
