...
- `tools.web.fetch.useTrustedEnvProxy` and `tools.web.fetch.ssrfPolicy`: retired `web_fetch`-local egress knobs. Use `proxy.enabled` and enforce private-network/SSRF policy in the external proxy instead.
