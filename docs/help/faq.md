...
<AccordionGroup>
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - If you are using the agent-facing `gateway` tool from an agent run, it will still reject writes to `tools.exec.mode` (including legacy `tools.bash.*` aliases that normalize to protected exec paths).
...
<AccordionGroup>
  ...
  <Accordion title="Is there an API / RPC way to apply config?">
    ...
    - The agent-facing `gateway` runtime tool still refuses to rewrite `tools.exec.mode`; legacy `tools.bash.*` aliases normalize to protected exec paths
