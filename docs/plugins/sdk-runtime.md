...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.state">
    ...
    `openChannelIngressQueue<TPayload>(...)` opens a persisted ingress queue scoped to the calling plugin. When stale-claim recovery uses `shouldRecover`, also provide `shouldRecoverCorrupt` if corrupt claimed payloads should be quarantined; its payload-independent claim identity preserves live owner and lane policy before tombstoning.
