...
When a sender-aware authorization policy is active, OpenClaw disables Codex's
native tool, environment, and multi-agent surfaces for that turn. Codex native
hooks are not a mandatory fail-closed boundary for every effect. OpenClaw
dynamic tools remain available and are authorized at the host-owned final
execution boundary.
...
| Native shell, patch, and MCP block or observe | Supported through native hook relay when no authorization policy is active       | Codex `PreToolUse` and `PostToolUse` are relayed for committed native tool surfaces, including MCP payloads on Codex app-server `0.142.0` or newer. Blocking is supported; argument rewriting is not. Sender-aware policy turns disable native tools and use final-gated OpenClaw dynamic tools instead.                                                                                                                                                                            |
