...
- Computer Use provider metadata for app-hosted macOS permission flows
...
| `contracts`                          | No       | `object`                         | Static capability ownership snapshot for external auth hooks, embeddings, speech, realtime transcription, realtime voice, media-understanding, Computer Use, image-generation, music-generation, video-generation, web-fetch, web search, and tool ownership. |
...
| `computerUseProviderMetadata`        | No       | `Record<string, object>`         | Cheap Computer Use host metadata for provider ids declared in `contracts.computerUseProviders`.                                                                                                                                                               |
...
## Computer Use provider metadata reference

`contracts.computerUseProviders` declares a plugin-owned Computer Use provider
id that OpenClaw can discover before loading plugin runtime. Use
`computerUseProviderMetadata` for cheap host-binding facts the macOS app can
inspect when deciding whether the provider can use OpenClaw.app-owned
permissions.

This is metadata only. It does not register tools, start a driver, grant macOS
permissions, or define a universal Computer Use action schema. The provider
plugin still owns its runtime mapping, including MCP servers or OpenClaw tools.
...
| Field           | Required | Type                                           | What it means                                                                                                                              |
| --------------- | -------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `platform`      | No       | `"darwin"`                                     | Platform where the provider can bind to an app-hosted Computer Use driver. Omit the field when the metadata is not platform-specific.      |
| `hostProtocol`  | No       | `string`                                       | Provider-owned protocol name or version for the host driver connection, for example a local daemon protocol.                               |
| `permissions`   | No       | `Array<"accessibility" \| "screen-recording">` | macOS permissions the provider expects the host app to hold for Computer Use operations.                                                   |
| `mcpServerName` | No       | `string`                                       | MCP server name the provider exposes for model-facing tools, when the Computer Use provider is surfaced through MCP rather than SDK tools. |
