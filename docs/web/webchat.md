...
- Reconnecting clients can send an optional `afterSeq` cursor to `chat.history` instead of refetching everything: first request `offset: 0` to seed a transcript-global `__openclaw.seq`, then pass the highest sequence already applied. The Gateway returns newer projected entries oldest-first, plus `nextAfterSeq` and `hasMore` for follow-up pages. Catch-up preserves display-projection context across raw page boundaries; under the byte budget, pages trim from the newest end and advance only past complete projected sequence groups, while fully hidden raw entries can advance without output. The cursor is scoped to the returned `sessionId`; clients must perform a full refresh when that id changes or `nextAfterSeq` moves behind the requested cursor. Sessions backed by imported CLI history return a normal full page without a cursor echo. `afterSeq` cannot be combined with `offset` pagination.
