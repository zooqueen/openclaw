...
- Reconnecting clients can send an optional `afterSeq` cursor to `chat.history` instead of refetching everything: pass the highest `__openclaw.seq` already seen and the Gateway returns only newer transcript entries oldest-first, plus `nextAfterSeq` and `hasMore` for follow-up pages. `afterSeq` cannot be combined with `offset` pagination.
