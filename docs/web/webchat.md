...
- Reconnecting clients can send an optional `afterSeq` cursor to `chat.history` instead of refetching everything: pass the highest `__openclaw.seq` already seen and the Gateway returns only newer transcript entries oldest-first, plus `nextAfterSeq` and `hasMore` for follow-up pages. Catch-up is lossless under the history byte budget: pages trim from the newest end and `nextAfterSeq` only advances past rows actually delivered, so trimmed rows arrive on the next page. `afterSeq` cannot be combined with `offset` pagination.
