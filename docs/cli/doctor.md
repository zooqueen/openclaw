...
- Doctor warns when compatibility `openai-codex/*` model refs remain in config. New config should use canonical `openai/*` refs, but `doctor --fix` preserves existing aliases and lets runtime resolution normalize them through the alias router. It can still remove stale whole-agent/session runtime pins and fallback notices that would force the wrong Codex harness.
