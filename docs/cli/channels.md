...
- `channels login` supports `--account <id>`, `--verbose`, and channel-specific login helpers such as WhatsApp `--phone-number <number>`; `channels logout` supports `--account <id>`.
