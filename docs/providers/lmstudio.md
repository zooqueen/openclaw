...
LM Studio plugin config uses the configured LM Studio endpoint for model requests, including loopback, LAN, and tailnet hosts. When managed proxy mode is enabled, allowlist that endpoint in the external proxy policy.
...
`lmstudio` uses its configured local/private endpoint for model requests. Custom/local OpenAI-compatible provider entries also use their exact configured `baseUrl` origin; managed-proxy deployments should allowlist those destinations in the external proxy policy.
