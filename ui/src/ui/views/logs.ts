...
export function renderLogs(props: LogsProps) {
  ...
  const exportText =
    exportLabel === "filtered" ? t("logsView.exportFiltered") : t("logsView.exportVisible");
    ...
    <section class="card">
      ...
      ${props.file
        ? html`<div class="muted" style="margin-top: 10px;">
            ${t("logsView.file", { path: props.file })}
          </div>`
        : nothing}
      ${props.truncated
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${filtered.length === 0
          ? html` <div class="muted" style="padding: 12px">${t("logsView.noEntries")}</div> `
          : filtered.map(
              (entry) => html`
                <div class="log-row">
