...
- Treat real behavior proof as the default for a behavior-changing PR. Put durable real-behavior evidence—not only tests, CI, or command/review comments—in the existing main-body **Evidence** section from the opening body. If proof is not yet available, state the concrete limitation and planned verification there; drafts can trigger automation, so update the body before ready and do not stack a re-review command after that update. When useful, use a compact prompt: behavior/issue, environment, after-patch steps, after-fix evidence/result, and relevant limitations; before evidence is encouraged for regressions. Redact private data. For docs-, test-, or behavior-neutral refactors, say why behavior proof is not applicable and list the relevant validation.
...
- Opening a PR, including a draft, plus pushes, body/title/base-branch edits, and ready-for-review transitions automatically queue a ClawSweeper review. Do not also post `@clawsweeper review` or `@clawsweeper re-review` after the same update; wait for its automatic review result or run instead. When available, a lifecycle/status comment is an early confirmation, not a prerequisite.
- A manual `@clawsweeper review` or `@clawsweeper re-review` command is appropriate only when no PR-content update has occurred since the prior review and an authorized requester needs a deliberate new review. Post it once, then wait for its response. CI/check completion alone does not queue a ClawSweeper review.
