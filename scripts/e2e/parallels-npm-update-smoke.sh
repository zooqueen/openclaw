...
refresh_fresh_summary_state() {
  local prefix="$1"
  local log_path="$2"
  local status_var="$3"
  local status_value="${!status_var:-skip}"

  if [[ ! -f "$log_path" && "$status_value" == "skip" ]]; then
    return 0
  fi
...
refresh_update_summary_state() {
  ...
  if [[ ! -f "$log_path" && ! -f "$status_path" && "$status_value" == "skip" ]]; then
    return 0
  fi
...
summary_has_activity() {
  ...
  [[ "$LINUX_FRESH_STATUS" != "skip" ]] && return 0
  [[ "$MACOS_UPDATE_STATUS" != "skip" ]] && return 0
  [[ "$WINDOWS_UPDATE_STATUS" != "skip" ]] && return 0
  [[ "$LINUX_UPDATE_STATUS" != "skip" ]] && return 0
  [[ -f "$RUN_DIR/macos-fresh.log" ]] && return 0
  [[ -f "$RUN_DIR/windows-fresh.log" ]] && return 0
  [[ -f "$RUN_DIR/linux-fresh.log" ]] && return 0
  [[ -f "$RUN_DIR/macos-update.log" ]] && return 0
  [[ -f "$RUN_DIR/windows-update.log" ]] && return 0
  [[ -f "$RUN_DIR/linux-update.log" ]] && return 0
  [[ -f "$(parallels_update_status_path "$RUN_DIR" macos)" ]] && return 0
  [[ -f "$(parallels_update_status_path "$RUN_DIR" windows)" ]] && return 0
  [[ -f "$(parallels_update_status_path "$RUN_DIR" linux)" ]] && return 0
  return 1
}
...
cleanup() {
  local exit_code=$?
  set +e
  if [[ "${SUMMARY_EMITTED:-0}" -eq 0 ]] && summary_has_activity; then
    emit_summary
  fi
...
emit_summary() {
  ...
  [[ "${SUMMARY_EMITTED:-0}" -eq 0 ]] || return 0
  summary_has_activity || return 0

  refresh_fresh_summary_state "MACOS_FRESH" "$RUN_DIR/macos-fresh.log" "MACOS_FRESH_STATUS"
  refresh_fresh_summary_state "WINDOWS_FRESH" "$RUN_DIR/windows-fresh.log" "WINDOWS_FRESH_STATUS"
  refresh_fresh_summary_state "LINUX_FRESH" "$RUN_DIR/linux-fresh.log" "LINUX_FRESH_STATUS"
  refresh_update_summary_state "MACOS_UPDATE" "$RUN_DIR/macos-update.log" "MACOS_UPDATE_STATUS" "macos"
  refresh_update_summary_state "WINDOWS_UPDATE" "$RUN_DIR/windows-update.log" "WINDOWS_UPDATE_STATUS" "windows"
  refresh_update_summary_state "LINUX_UPDATE" "$RUN_DIR/linux-update.log" "LINUX_UPDATE_STATUS" "linux"
