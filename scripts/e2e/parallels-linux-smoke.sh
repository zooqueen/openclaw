...
print(
    json.dumps(
        {
            "content": f"parallels-linux-smoke-inbound-{os.environ['NONCE']}",
            "flags": 4096,
        }
    )
)
...
if [[ "$JSON_OUTPUT" -eq 1 ]]; then
  ...
  printf '  latest->main: %s (%s) dashboard=%s discord=%s\n' "$UPGRADE_STATUS" "$UPGRADE_MAIN_VERSION" "$UPGRADE_DASHBOARD_STATUS" "$UPGRADE_DISCORD_STATUS"
  printf '  logs: %s\n' "$RUN_DIR"
  printf '  summary: %s\n' "$SUMMARY_JSON_PATH"
fi
