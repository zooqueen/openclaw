...
- xAI/tool-schema compatibility: strip xAI-unsupported JSON Schema validation keywords from tool definitions (including OpenRouter `x-ai/*` model refs) so tool-enabled turns no longer fail with invalid-arguments schema errors. (#32080) Thanks @asyncjason.
