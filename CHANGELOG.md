...
- OpenAI Codex/auth: derive provider-owned OAuth profile ids from JWT identity claims while keeping fallback profile ids out of email metadata, so missing `creds.email` no longer collapses distinct logins onto `openai-codex:default`. (#53825) Thanks @pober-life.
