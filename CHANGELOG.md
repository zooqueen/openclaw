...
- Plugin SDK/facades: back-fill facade sentinels before plugin-id tracking and tracked-plugin resolution re-enter config loading, so CLI/provider startup no longer crashes with empty-facade reads during bundled plugin normalization loops. (#61180) Thanks @adam91holt.
