...
- Cron/isolated-agent test stability: assert provider/model + prompt overrides against the most recent embedded run invocation in cron isolated-agent tests to avoid Bun-specific call-order flakes during retry/fallback paths. (#32287) Thanks @ningding97.
