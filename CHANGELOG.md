...
- Tools/image generation: deprecate the bundled `openai-image-gen` skill, standardize on the native `image` and `image_generate` tools, and clarify provider-specific image auth requirements in runtime output and docs.
