...
- Feishu/Bitable tools: resolve credentials through multi-account config (`channels.feishu.accounts.*`) so bitable tools register correctly outside legacy top-level `appId`/`appSecret`, with regression coverage for both account shapes. (#27261) thanks @echoVic.
