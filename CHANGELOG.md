...
- Agents/web_search: only prefer runtime-selected non-bundled providers while that provider is still present in the active plugin registry, so stale runtime web metadata after plugin churn does not silently switch `web_search` to a different runtime plugin. Thanks @jzakirov.
