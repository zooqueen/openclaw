...
- Telegram/DM peer isolation: prefer sender user id over `chat.id` for direct-session routing keys when they differ, preventing cross-user DM session collisions in `per-channel-peer` scope while preserving `chat.id` fallback when sender id is unavailable. (#32207) Thanks @bmendonca3.
