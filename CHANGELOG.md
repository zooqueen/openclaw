...
- Telegram/inbound media filenames: preserve original `file_name` metadata for document/audio/video/animation downloads (with existing fetch/path fallbacks), so saved inbound attachments keep sender-provided filenames instead of opaque Telegram file paths. (#31837) Thanks @Kay-051.
