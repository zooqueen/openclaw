...
- Agents/Forward-compat models: extend `gpt-5.3-codex` forward-compat fallback eligibility to `github-copilot` (in addition to `openai-codex`) so Copilot users can select `github-copilot/gpt-5.3-codex` before upstream catalog updates land. (#27542) thanks @Sid-Qin.
